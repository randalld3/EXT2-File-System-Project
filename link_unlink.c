...
int truncate(MINODE *mip)
{
    ...
    if (mip->INODE.i_block[12]){ // indirect block exists
        get_block(dev, mip->INODE.i_block[12], (char *)ibuf); 
        i = 0;

        while(ibuf[i] && i < 256){
            bdalloc(dev, ibuf[i]); // dealloc indirect blocks from block
            ibuf[i] = 0; // set indirect block to 0, showing dealloc
            i++;
        }

        bdalloc(dev, mip->INODE.i_block[12]); // deallocate indirect block
        mip->INODE.i_block[12] = 0; // set indirect block to 0
    }

    if (mip->INODE.i_block[13]){ // double indirect block exists
        get_block(dev, mip->INODE.i_block[13], (char *)dbuf);
        i = 0;
